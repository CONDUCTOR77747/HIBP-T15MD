...
from hibpy.calc.grid import Grid
...
class FieldInterpolator():
    ...
    def plot(self, color='r', dens=1.0, plot_sep=True):
        ...
        ax1.streamplot(x, y, Fx[:, :, z_cut].swapaxes(0, 1), Fy[:, :, z_cut].swapaxes(0, 1), color=color, density=dens, linewidth=0.5)
        ax2.streamplot(x, z, Fx[:, y_cut, :].swapaxes(0, 1), Fz[:, y_cut, :].swapaxes(0, 1), color=color, density=dens, linewidth=0.5)
        plt.show()
        
    def plot_levels(self,  color='r', dens=1.0, plot_sep=True, levels=31): #!!!
        '''
        contour plot of magnetic field
        '''
        ...
        for i in np.arange(len(x)):
            for j in np.arange(len(y)):
                for k in np.arange(len(z)):
                    F[i, j, k] = np.linalg.norm([Fx[i, j, k], Fy[i, j, k], Fz[i, j, k]])
            
        ax1.contour(x, y, F[:, :, z_cut].swapaxes(0, 1), levels=levels)
        ax2.contour(x, z, F[:, y_cut, :].swapaxes(0, 1), levels=levels)
        plt.show()
        return F
...
def read_B_new(Btor, Ipl, PF_dict, dirname='magfield'):
    ...
    for filename in os.listdir(dirname):
        ...
        elif 'Plasm_{0:.1f}MA'.format(Ipl) in filename:
...
#%%
def normalize(B_tor, Btor_on_axis, grid, n_coils=16):
    Bx = B_tor[:, :, :, 0]
    By = B_tor[:, :, :, 1]
    Bz = B_tor[:, :, :, 2]
    B_int = FieldInterpolator(grid, [Bx, By, Bz])
    
    phi = np.pi/n_coils
    step=phi/100
    r = [1.48, 0., 0.]
    B_tmp = []
    for theta in np.arange(-phi, phi+step, step):
        r2 = [r[0]*np.cos(theta), 0., r[0]*np.sin(theta)]
        B_tmp.append(np.linalg.norm(B_int(r2)[0]))
    B_avg = sum(B_tmp)/len(B_tmp)
    
    norm_coeff = Btor_on_axis/B_avg
    norm_B_tor = B_tor*norm_coeff*-1        # reversing current so field will be orientated properly. should move it to calculation though
    return norm_B_tor

def read_B_beta(Btor_on_axis, Jpl, PF_currents_dict, dirname='magfield'):
    SI_1MA = 1e6
    dirname += f'\\Jpl_{Jpl}MA\\'
    grid = Grid.from_file(dirname + 'geometry.dat').grid
    print("Reading magnetic field")
    
    print(f"Reading toroidal field with {Btor_on_axis} T on axis")
    with open(dirname + 'Btor.npy', 'rb') as f:
        B_tor = np.load(f)
        B_tor = normalize(B_tor, Btor_on_axis, grid)
        
    print(f"Reading plasma field for {Jpl} MA plasma current")
    with open(dirname + 'Bpl.npy', 'rb') as f:
        B_pl = np.load(f)
    
    B_pol = None
    for name in PF_currents_dict.keys():
        print(f"Reading field of {name} coil with current {PF_currents_dict[name]} MA")
        with open(dirname + f'{name}.npy', 'rb') as f:
            B_pol_tmp = np.load(f)
            B_pol_tmp *= PF_currents_dict[name]*SI_1MA # norm on current
            B_pol = B_pol_tmp if (B_pol is None) else (B_pol + B_pol_tmp)
    
    B_tot = B_tor + B_pl + B_pol
...
def plate_flags(range_x, range_y, range_z, U,
    ...
    for i in range(UP_full.shape[0]):
        UP_rotated[i, :] = rotate(UP_rotated[i, :], axis=(1, 0, 0), deg=gamma) # gamma == 0 or gamma == 90 deg
        # shift coords center
...
def save_E(beamline, plts_name, Ex, Ey, Ez, plts_angles, plts_geom,
    ...
    with open(dirname + '/' + fname, 'w') as myfile:
        ...
        myfile.write(np.array2string(domain, max_line_width=200)[1:-1] + ' # xmin, xmax, ymin, ymax, zmin, zmax, delta\n')

        if plts_name == 'an':
            myfile.write(np.array2string(an_params, max_line_width=200)[1:-1] + ' # n_slits, slit_dist, slit_w, G, theta, XD, YD1, YD2\n')
