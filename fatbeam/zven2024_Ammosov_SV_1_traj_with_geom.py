...
if plot_lines:
    # plot primary trajectory
    plot_prim(tr, ax, axes='XY', color='k', full_primary=True, alpha=1)
...
for i in slits:
    ...
    for fan_tr in tr.RV_sec_toslits[i]:
        if plot_lines:
            ax.plot(fan_tr[:, 0], fan_tr[:, 1], fan_tr[:, 2], color=c, alpha=1)
            
        if plot_dots:
            # plot zones
            ax.plot(fan_tr[0, 0], fan_tr[0, 1], fan_tr[0, 2], '.', color='white',
                      markerfacecolor='red', alpha=1, markersize=40)

x_rv0 = []
y_rv0 = []
z_rv0 = []

x_rv0.append(tr.RV0[0][0])
y_rv0.append(tr.RV0[0][1])
z_rv0.append(tr.RV0[0][2])

ax.scatter(tr.RV0[0][0],tr.RV0[0][1],tr.RV0[0][2], color='red')
...
if plot_geom:
    ...
    for i in range(n_slits):
        x_slit = list(r_slits[i, 1:, 0])
        y_slit = list(r_slits[i, 1:, 1])
        z_slit = list(r_slits[i, 1:, 2])
...
if viewpoint=='start':
    factor = 0.11
    ax.set_xlim(min(x_rv0)-factor, max(x_rv0)+factor)
    ax.set_ylim(min(y_rv0), max(y_rv0))
    ax.set_zlim(min(z_rv0), max(z_rv0))
    ax.set_aspect('equal')
    ax.view_init(elev=135., azim=-85., roll=0.)
    
elif viewpoint=='iz':
    ax.set_xlim(x_min/100, x_max/100)
    ax.set_ylim(y_min/100, y_max/100)
    ax.set_zlim(z_min/100, z_max/100)
    ax.view_init(elev=128., azim=-55., roll=0.)
    
elif viewpoint=='slits':
    factor = 0.15
    ax.set_xlim(min(x_slit)-factor, max(x_slit)+factor)
    ax.set_ylim(min(y_slit), max(y_slit))
    ax.set_zlim(min(z_slit), max(z_slit))
    ax.set_aspect('equal')
    ax.view_init(elev=139., azim=-25., roll=0.)
